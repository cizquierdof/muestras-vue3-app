...
export const useSiteStore = defineStore({
  ...
  getters: {
     ...
     */
    //Devuelve DOCUMENTO_NO-WEB si Type=DOWNLOADABLE-DOC
    Clase: (state) => {
      return state.inType === "DOWNLOADABLE_DOC"
        ? "DOCUMENTO_NO_WEB"
        : "PAGINA_WEB";
    },
    //Devuelve número de urls de la muestra
    numUrl: (state) => {
    ...
    //Devuelve número de páginas aleatorias
    numAleatory: (state) => {
      return state.siteWebPages.reduce((acc, e) => {
  ...
  actions: {
    ...
    saveJson() {
      ...
      link.href = blobUrl;
      link.download = this.siteName + `_v2.json`;
